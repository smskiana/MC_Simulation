...
namespace WorldCreatation
{
    ...
    public class World : MonoBehaviour
    {
        ...
        private (int x, int z) lastUpdatePos;
        private List<(int x, int z)> oldChunkValue;
        ...
        public bool HaveInit { get => haveInit;  }

        #endregion
        #region 事件
        public event Action InitFinish;
        #endregion
        ...
        public bool CancelDrawFace(Vector3 pos, int face)
        {
            ...
            if (haveInit&&TryGetChuck(pos, out var chu)&& !lockedChucks.Contains(chu.pos))
            {
        ...
        public bool AddBlock(BlocksInfo BlockInfo , Vector3Int pos)
        {
            if(TryGetChuck(pos,out var chuck))
            {
                return chuck.AddBlo(BlockInfo.ID,chuck.ToChunkPos(pos));
            }
            return false;
        }
        public bool RemoveBlock(Vector3Int pos)
        {
            if (TryGetChuck(pos, out var chuck))
            {
               return chuck.RemoveBlo(chuck.ToChunkPos(pos));
            }
            return false;
        }
