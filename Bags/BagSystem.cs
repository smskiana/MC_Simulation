...
public class BagSystem : MonoBehaviour
{
    ...
    private class BagCell : IComparable<BagCell>,IEquatable<BagCell>
    {
        ...
        public int Add(BagCell pair)
        {
           ...
           return Add(pair.item,pair.Count);
        }
        public int Add(ItemStat item,int count)
        {
            ...
            {
                countbuf = this.Count + count;
            }
            else
            {
                return count;
            }
            int rest = countbuf - item.ItemInfo.MaxStackCount;
            if (rest>0)
            {
        ...
        public bool Equals(int SourceID) => item.Equals(SourceID);
        public bool Equals(ItemStat other) => Item != null && Item.Equals(other);
    ...
    #endregion
    [Button("初始化")]
    public void Init(int size=-1)
    {
        if(size > 0)this.size = size; 
        mask ??= BagLayMask.AllContainMask;
#if UNITY_EDITOR
        Mask = Convert.ToString(mask.Mask, 2);
#endif
        bagItems ??= new List<BagCell>(this.size);
        foreach (BagCell cell in bagItems)
        {
            cell.CellStatChange += OnCellStatChange;
        }
        while (bagItems.Count < this.size)
        {
            bagItems.Add(new BagCell(OnCellStatChange));
        }
        this.size = bagItems.Count;
    ...
    {
        if (pos < 0 || pos >= size) return (null,0);
    ...
    public bool TryTakeAnyStrict(int Sourceid, int count, out List<(ItemStat item, int count)> result)
    {
        result = new List<(ItemStat item, int count)>();
        if (Sourceid == 0 || count <= 0) return false;
