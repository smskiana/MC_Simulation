    ...
    public class Chuck
    {
        ...
        private void InitAdd(Vector3Int pos, int id  = 0)
        {
            ...
            if (!blocks.ContainsKey(pos))
            {
                ...
                if (!SubmeshIndex.TryGetValue(id, out var p))
                {
                    ...
                    if (InfoStorer.Instance.TryFindBlock(id, out var s))
        ...
        private void RePlaceFace(int sub, Block block, int face,int resub,Block reblock, int reFace)
        {
            ...
            if (sub != resub)
            {
                ...
                //如果子网格再无数据
                if (subMeshInfo[resub].blocks.Count == 0 && subMeshInfo[resub].triangles.Count==0)
                {
                    ReMoveSubTris(resub);
                    return;
                }
        ...
        /// 标记要添加的方块
        /// </summary>
        public bool AddBlo(int ID, Vector3Int pos)
        {
            if (!IsFreeSpace(pos)) return false;
            if (!blocks.ContainsKey(pos))
            {
                Block block = BlockFactory.GetInstance().GetBlock(ID, pos);
                ...
                if (!SubmeshIndex.TryGetValue(ID, out int posInSubmesh))
                {
                    ...
                    if(InfoStorer.Instance.TryFindBlock(ID,out var info))
        ...
        /// 标记要移除的方块
        /// </summary>
        public bool RemoveBlo(Vector3Int pos)
        {
            if (blocks.TryGetValue(pos, out var block))
            {
