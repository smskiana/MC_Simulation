...
public class BagSystemUI : MonoBehaviour
{
    ...
    [SerializeField] private SplitUI SplitPanel;
    ...
    public BagSystem BagSystem { get =>
             blindBagSystem; set => blindBagSystem = value;
         }
    ...
    public void MoveAndShowSplitPane(Vector2 pos,int posinbag)
    {
        ...
        int maxCount = count;
        SplitPanel.Open(maxCount / 2, maxCount);
    }
       ...
       if(splitCellPos >= 0)
        {
            if (BagSystem != null)
            {
                BagSystem.Split(splitCellPos, SplitPanel.Value);
            }
        }
        splitCellPos = -1;
    }
